...
imgPath = r"D:\@work\Python\DeepLearning\resource\customImage\ice cream2.jpg"
...
def Predict(model):
    ...
    predClass = foodCategory[tf.argmax(pred[0])]
    prob = np.around(tf.squeeze(pred), 3)
    print(predClass)
    plt.imshow(imgRaw)
...
def main():
    model = tf.keras.models.load_model("savedModel/food101_MultiClass_2")
    model.summary()
    Predict(model)
    plt.show()
    pass
